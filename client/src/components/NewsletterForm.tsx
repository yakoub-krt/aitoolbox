...
export default function NewsletterForm({ compact = false }: { compact?: boolean }) {
  ...
  const subscribe = trpc.newsletter.subscribe.useMutation({
    onSuccess: () => {
      setEmail("");
      setMessage("تم تسجيل اشتراكك. ستصلك أحدث مقالات AIToolBox عند نشرها.");
    },
  ...
  return <div className={compact ? "mt-5" : "relative overflow-hidden rounded-[2rem] border border-violet-300/20 bg-gradient-to-l from-violet-500/16 via-[#15132f] to-[#0b1022] p-7 md:p-10"}>
    ...
    <div className={compact ? "" : "relative max-w-2xl"}>
      ...
      <p className={compact ? "mt-2 text-xs leading-6 text-slate-400" : "mt-3 text-sm leading-7 text-slate-300"}>{compact ? "رسالة عند نشر مقال جديد، ويمكنك إلغاء الاشتراك في أي وقت." : "سجل بريدك لنرسل لك تحديثاً تلقائياً عند نشر دليل جديد أو مقارنة عملية. يمكنك إلغاء الاشتراك متى شئت."}</p>
      <form onSubmit={submit} className={compact ? "mt-4 flex gap-2" : "mt-6 flex flex-col gap-3 sm:flex-row"}>
        ...
        <Button disabled={subscribe.isPending} className="h-11 bg-violet-500 px-5 text-white hover:bg-violet-400">{subscribe.isPending ? "جارٍ التسجيل…" : "اشترك مجاناً"}</Button>
      </form>
      <p className="mt-3 text-[11px] leading-5 text-slate-500">بتسجيلك، توافق على تلقي تحديثات AIToolBox وفق <Link href="/privacy" className="text-violet-300 underline underline-offset-2">سياسة الخصوصية</Link>.</p>
