...
try {
  ...
  if (clipboardText !== reelOutput) throw new Error("Customizer output was not copied to the clipboard.");

  await page.screenshot({ path: "/home/ubuntu/prompt-customizer-browser-check.png", fullPage: true });
  console.log(JSON.stringify({ productVideo: true, productAd: true, educationalReel: true, signupCallout: true, copiedLength: clipboardText.length }));
  await context.close();
} finally {
  await browser.close();
}
